...
#   stage 11 mldsa_c2sp_kat_gate.sh         ML-DSA-44/65/87 reproduce the OFFICIAL C2SP/CCTV FIPS-204
#                                           accumulated digests (100- AND 10000-iter checkpoints,
#                                           keygen+sign+verify, three parameter sets in parallel;
#                                           F follow-on)
#
...
# Exit:   0 = GATE GREEN (all eleven stages)
#         N = 1..11, the first red stage
...
stage11_mldsa_c2sp() {
    bash "$III_ROOT/STDLIB/scripts/mldsa_c2sp_kat_gate.sh"
}
...
run_stage 11 "ML-DSA-44/65/87 official C2SP FIPS-204 KATs (mldsa_c2sp_kat_gate.sh)" "$LOGDIR/11_mldsa_c2sp.log" stage11_mldsa_c2sp

log "GATE GREEN: full toolchain regenerated from clean, sovereignly sealed, basal census green, crypto closure witness-zero, sovereign toolchain self-mints gcc-free, ML-KEM byte-FIPS-203-conformant, ML-DSA byte-FIPS-204-conformant (logs: $LOGDIR)"
exit 0
