...
# (mhash_lib.sh).  REQUIRED: this script links the stdlib archive, so the
# sovereign hasher is mintable wherever this script can succeed.  At bootstrap
# stage 4 the committed iiis-2.exe is already deleted -- mhash_lib mints via the
# freshly golden-verified iiis-1.exe (its sealed-compiler candidate list).
