...
extern uint64_t L_sanctum_do_ord_binop(uint64_t x) __attribute__((sysv_abi));
...
int main(void) {
    ...
    if (L_sanctum_do_ord_binop(0x8000000000000001ULL) != 1) return 4; /* R2: unsigned u64 ordering on a typed-binary operand (lhs-else-rhs class) */
    return 21;   /* all four pass -> exit 21 (the gate's expected value) */
}
