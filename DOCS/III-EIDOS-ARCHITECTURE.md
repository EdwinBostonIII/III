...
- **ADR-6 — The Composer's proposer is domain-specific; nous is split into two faces (2026-06-21).** The cost-graph Composer's proposer is the deterministic cost-argmin (Bellman-Ford), not nous. *Rationale (from nous's source, §3.4.1):* `nous_value`/`nous_policy` rank **rewrite rules/terms** by rewrite-domain features toward a normal form — faithful only to a *rewrite-domain* Composer, not a cost-graph one; forcing them onto quanta is a wrong-faculty island. The faithful nous⇄Composer link is **`nous_costlin`** (the canonical, versioned total order over the shared `cost_lattice`), which grounds optimality in a total order — **identified as the integration-phase wiring, not done now** (deferred per review: wiring it onto today's fixture would polish a disconnected mechanism and be re-designed when the cost model is real). *Consequence:* the "nous deferred" note is corrected to "nous-proposer correctly scoped to the rewrite domain; `nous_costlin` is the total-order integration point." *Alternative rejected:* encode quanta as XII terms so nous-the-proposer ranks them (the wrong-faculty island).
  - **RESOLUTION (reunification W4c.2.1, 2026-07-02) — STRIKE the wire-now option, standing condition preserved.** Decided by reading `eidos/compose.iii`: it holds NO `nous_costlin` extern (the prose-only branch of the wire-or-strike decider), so the integration point is not yet a live edge. And the durably-gated finding `1937_eidos_weave_subadditive` proves the real `xii_savings` geometry is **sub-additive** ⇒ the optimal plan is **order-invariant under every monotone order**. A `nous_costlin` wiring KAT therefore *cannot* exhibit differential (two-path) teeth on today's geometry — wiring now would be a **tautological gate** (forbidden by the no-tautological-proofs standard). The ADR's own condition ("re-designed when the cost model is real") stands as-is; `nous_costlin` remains the identified integration point for when a richer per-ripple cost model carries trade-off dimensions the savings table does not. No code change; the wire-now branch is struck with this reason + the 1937 citation.
...
| `eidos/optgate` (Phase 3 Task A: cost-ordered battery-skip) | **NEW — PROVEN** (corpus `1938` EXIT 99: wakes the DORMANT SAT reducer `invent/gil_forge` cost-optimally; skip MEASURED on the power-of-two simple case, correctness `bb_equal`-proven; the prior over-invoke on irreducible `k` is **FIXED** via `invent/gil_reducible` — a cheap no-SAT pre-check proven sound both ways (`1948`); discernment is now *reducibility*; validates the **cost-selection half**, not Bellman-Ford; sealed + wired) |
...
| `eidos/anchor` — self-location (the keystone, §9) | **NEW — PROVEN (identity + verification half)** (corpus `1944` EXIT 99: WRAPS the proven unprivileged Census Crystal — content-addressed identity, byte-identical (FR-2) + observe-only `anchor_verifies`/`anchor_has_feature` reachability via census facts + real CPUID. **SAFE BY CONSTRUCTION: ZERO metal, NO write function → the KATABASIS §0.6 bricking-write is unrepresentable.** The ring-descent writes stay OPEN-CORE north-star (declared, not executed); sealed + wired) |
| `eidos/orchestrate` — host-adaptive composition | **NEW — PROVEN as a CAPABILITY** (corpus `1945`/`1946` EXIT 99: CALLS `eidos/compose` gated by `anchor` — a capability the host can't verify is EXCLUDED, so the plan reshuffles around it; cross-host divergence PROVEN via `cpufeat_force_mask`. **Honest:** NOT a live consumer — nothing but tests call it (still **zero live consumers**); reshuffle is one bit per edge; a real caller (`tp_*`) is the open gap) |
| `eidos/field` — unified spatial+temporal field (FR-1) | **NEW — PROVEN** (corpus `1947` EXIT 99: ONE interface — SPATIAL gradient (`ripple_field`) + TEMPORAL fold (`event_substrate`) + branch-retaining REWIND (`dome`). **Retire-by-attrition BLOCKED** — the candidates (`omnia/ripple`,`forcefield/ripple`) have LIVE CONSUMERS (`weave`/`ripple_journal`/`sov_isa`), so the carto zero-consumer proof FAILS; deletion would break the build — correctly NOT done) |
| `eidos/anchor` ring-descent / "melt in → host runs faster" | **OPEN CORE / NORTH-STAR** (§9.3, burden-of-proof; the actual metal writes, never assumed) |
