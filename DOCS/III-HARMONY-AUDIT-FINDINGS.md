...
| 3 | 50 | **`tp_planner.iii`** — Pareto-frontier format-transform planner (BFS over the tp_* morphism graph; non-dominated routes by composed 6-vector cost) — **DONE 2026-06-11**: `omnia/tp_planner.iii` (deterministic simple-path DFS over every COSTED arrow of the one category; hop bound == CATEGORY_WORDCAP by law; saturating CC_SAT cost composition; pe frontier + cl_dot_slot declared-order pick; `tpp_materialize` = the category certifies each route via cat_compose, empty route = cat_identity; refuse-whole on pool overflow). KAT `corpus/1390_tp_planner.iii` = 99 (87 arms incl. K7 326-path refusal + cl_dominates cross-authority); standalone-gated vs /tmp lib snapshot (writer active); TEETH PROVEN: sabotaged wrap-adder exits 61, sabotaged silent-truncation exits 87. WIRE-IN PENDING (writer-clear): build_stdlib MODULES + run_corpus EXPECTED [1390_tp_planner]=99. | tp_morphism ⊗ category ⊗ pareto_extraction ⊗ cost_lattice | **[SAFE-NOW]** |
