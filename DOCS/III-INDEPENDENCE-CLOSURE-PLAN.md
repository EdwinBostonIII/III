...
> | F PQ NIST KAT (ML-KEM) | **DONE — module FIXED + gated** | corpus/2490 is the official C2SP/CCTV FIPS-203 ML-KEM-768 accumulated KAT (Go crypto/BoringSSL gate on it). It initially REVEALED a real divergence; localized via the C2SP `intermediate/ML-KEM-768.txt` trace (68 KB, fetched) to K-PKE.KeyGen's `G` input: the ported Kyber-round-3 code appended the dimension byte (`G(d‖k)`), but FIPS-203 ML-KEM uses `G(d)` with NO trailing byte (`SHA3-512(d)` == ρ‖σ exactly). Fixed in `mlkem.iii`; the module now reproduces the official digest `f7db260e…` byte-for-byte over 10000 iters. Wired as **bootstrap stage 10**. (ML-DSA/SLH-DSA C2SP KATs remain as follow-on PQ conformance.) |
