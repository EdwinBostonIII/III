...
### The bounded Gröbner-degree experiment — BUILT (KAT 1847)
"Mastering it ourselves": the algebraic-geometry survivor (Nullstellensatz / elimination-ideal) is now wired into III's own Gröbner engine and validated. Encoding (over **GF(3)**): root r, "Even wins r" ⟺ ∃ Even positional σ making the σ-restricted game Even-won at r (positional determinacy); the winning predicate p(σ) is computed exactly by the verified Zielonka solver, its **signed multilinear extension** is the polynomial, and ∃σ p(σ)=1 ⟺ the ideal ⟨p−1, x_i²−x_i⟩ has a reduced Gröbner basis ≠ {1}. `gb_buchberger` decides it; every verdict is **cross-checked against the oracle**.
- **Result: the merge is REAL and CORRECT** — III's Gröbner == the oracle on *every* k≤2 game (1500 trials) + all 16 exhaustive k=2 truth tables + hand cases; max basis leading degree = 2 (the field equations).
- **Found along the way:** (a) GF(2) is unsupported by `galois` (`gfp_inv(1,2)`→E_INV; the engine's KAT only ever used GF(7)) — use GF(3). (b) The **binding limit is not the 16-var cap I predicted — it is III's 64-slot bigint handle table**: `gb_init` never frees coeff bigints (handles leak across calls), and a *single* Buchberger run at k≥3 itself creates >64 intermediate handles mid-computation → degenerate handles → wrong verdicts (34 mismatches at k≤5). Freeing the basis+generators+prime per call keeps it correct at k≤2; the degree-**growth** curve (k≥3) needs a larger handle table.
- **Verdict:** the algebraic survivor *is* implementable on III and verifiably correct, but the engine caps it below where degree growth would appear — exactly the dichotomy's prediction (the machinery connects; it is not a breakthrough; the worst-case degree question = "is parity in P?" remains untouched).
