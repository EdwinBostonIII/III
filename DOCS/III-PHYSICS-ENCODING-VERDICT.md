...
6. **Verdict.** **VERIFIED-IN-CODE** for the conservation theorem and the memory seal: `2073` → 99 and `2074` → 99 against the in-tree `iiis-2` (the binary ran it, not self-attested), with the dual-engine cross-check executing and the conscience/leak/overlap negatives firing. The two **apex wave gates** (`svp_conservation`, `svp_mem_seal`) are **VERIFIED-BY-EQUIVALENCE** (identical primitives + examples as `2073`/`2074`); their only-via-`2036` exit-check is blocked by the pre-existing ~1.25 GB-BSS `exit 126` load failure — an environment limit, not an undischarged hypothesis.
