#!/usr/bin/env bash
# Verify huffman.iii consumers (direct + transitive via lzh) + the new falsifier
# against the REBUILT (guarded) lib.  All must exit 99.
set -u
...
TESTS=(1506_huffman_decode_len_oob 1230_huffman 1231_lzh)
ALLOK=1
for t in "${TESTS[@]}"; do
  SRC="$ROOT/STDLIB/corpus/$t.iii"; OBJ="$WORK/$t.o"; EXE="$WORK/$t.exe"
...
echo "----"
[[ $ALLOK -eq 1 ]] && echo "ALL HUFFMAN CONSUMERS GREEN" || echo "*** REGRESSION ***"
