...
extern uint32_t p_run_n_rounds_get(uint32_t n, uint32_t idx);
static uint32_t rotr(uint32_t x, uint32_t n) { return (x>>n) | (x<<(32-n)); }
int main(void) {
    /* C reference state machine */
    ...
    for (int n = 1; n <= 2; n++) {
        ...
        printf("After %d round(s):\n", n);
        for (int i = 0; i < 8; i++) {
            p_init_state();
            uint32_t iii_v = p_run_n_rounds_get(n, i);
            uint32_t c_v = reg[i];
            printf("  %c: iii=0x%08X  c=0x%08X  %s\n", names[i], iii_v, c_v, iii_v == c_v ? "OK" : "DIFF");
        }
    }
    return 0;
}
