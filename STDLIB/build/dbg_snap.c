#include <stdio.h>
#include <stdint.h>
extern uint32_t q_init(void);
extern uint32_t q_rounds_snapshot(uint32_t n, uint32_t idx);
...
int main(void) {
    ...
    for (int n = 1; n <= 3; n++) {
        ...
        printf("After %d round(s) [snapshot version]:\n", n);
        int all_ok = 1;
        for (int i = 0; i < 8; i++) {
            q_init();
            uint32_t iii_v = q_rounds_snapshot(n, i);
            uint32_t c_v = reg[i];
