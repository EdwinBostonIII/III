# III Stage-0 Ring-3 codegen output
# Microsoft x64 ABI; gas syntax (AT&T); PE/COFF target
    .att_syntax
    .file 1 "<iii-source>"
    .section .rodata
L_str_0:
    .ascii "pattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_1:
    .ascii "pattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_2:
    .ascii "pattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_3:
    .ascii "pattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_4:
    .ascii "pattern.iiipattern.iiipattern.iiipattern.iiipattern.iiipattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_5:
    .ascii "pattern.iiipattern.iiipattern.iiipattern.iiipattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_6:
    .ascii "pattern.iiipattern.iiipattern.iiipattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_7:
    .ascii "pattern.iiipattern.iiipattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_8:
    .ascii "pattern.iiipattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_9:
    .ascii "pattern_table.iiitp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_10:
    .ascii "tp_iii_to_asm.iiitp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_11:
    .ascii "tp_iii_to_babel_json.iiitp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_12:
    .ascii "tp_babel_json_to_iii.iiitp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_13:
    .ascii "tp_asm_to_pe.iiitp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_14:
    .ascii "tp_ast_bin_to_iii.iiitp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_15:
    .ascii "tp_babel_cbor_json.iiitp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_16:
    .ascii "tp_babel_json_cbor.iiitp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_17:
    .ascii "tp_babel_text.iiitp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_18:
    .ascii "tp_ripple_dot.iiitp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_19:
    .ascii "tp_iii_to_ast_bin.iiitp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_20:
    .ascii "tp_iii_to_c99.iiitp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_21:
    .ascii "tp_iii_to_latex.iiitp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_22:
    .ascii "tp_x86_disasm.iiitp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_23:
    .ascii "tp_pe_hex.iiitp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_24:
    .ascii "tp_ripple_md.iiitp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_25:
    .ascii "tp_ast_dot.iiitp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_26:
    .ascii "tp_babel_text_back.iiitp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_27:
    .ascii "tp_iii_hex.iiitp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_28:
    .ascii "tp_c99hdr_to_iii.iiitp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_29:
    .ascii "tp_iii_to_md.iiitp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_30:
    .ascii "tp_x86_assemble.iiitp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_31:
    .ascii "tp_raw_hex.iiitp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_32:
    .ascii "tp_ast_to_babel_json.iiitp_babel_json_to_ast.iii\0"
L_str_33:
    .ascii "tp_babel_json_to_ast.iii\0"
    .section .bss
    .global L_TX_MODULE_MHASH
L_TX_MODULE_MHASH:
    .zero 256
    .global L_TX_TEMPLATE_BUF
L_TX_TEMPLATE_BUF:
    .zero 1344
    .global L_TX_NAME_BUF
L_TX_NAME_BUF:
    .zero 64
    .global L_g_transform_fn_table
L_g_transform_fn_table:
    .zero 4608
    .section .data
    .global L_TX_REG_FAIL
L_TX_REG_FAIL:
    .quad 0x0
    .section .iii.ring3,"n"
    .asciz "tp_table_set"
    .text
    .global L_tp_table_set
    .seh_proc L_tp_table_set
L_tp_table_set:
    pushq %rbp
    .seh_pushreg %rbp
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    subq $1024, %rsp
    .seh_stackalloc 1024
    .seh_endprologue
    movq %rcx, -8(%rbp)
    movq %rdx, -16(%rbp)
    movq %r8, -24(%rbp)
    movq %r9, -32(%rbp)
    movl -8(%rbp), %eax
    pushq %rax
    movabsq $0x18, %rax
    pushq %rax
    popq %rcx
    popq %rax
    cmpq %rcx, %rax
    setae %al
    movzbq %al, %rax
    pushq %rax
    popq %rax
    testq %rax, %rax
    jz L_if_end_1
    movabsq $0x1, %rax
    pushq %rax
    popq %rax
    negq %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    popq %rax
L_if_end_1:
    leaq L_g_transform_fn_table(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -40(%rbp)
    movl -8(%rbp), %eax
    pushq %rax
    popq %rax
    pushq %rax
    movabsq $0xff, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    movabsq $0x18, %rax
    pushq %rax
    popq %rcx
    popq %rax
    imulq %rcx, %rax
    pushq %rax
    popq %rax
    movq %rax, -48(%rbp)
    movq -40(%rbp), %rax
    pushq %rax
    movq -48(%rbp), %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -56(%rbp)
    movq -40(%rbp), %rax
    pushq %rax
    movq -48(%rbp), %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -64(%rbp)
    movq -56(%rbp), %rax
    pushq %rax
    movabsq $0x0, %rax
    pushq %rax
    movzbq -16(%rbp), %rax
    pushq %rax
    popq %rdx
    popq %rcx
    popq %rax
    movb %dl, (%rax,%rcx,1)
    movq -64(%rbp), %rax
    pushq %rax
    movabsq $0x0, %rax
    pushq %rax
    movzbq -24(%rbp), %rax
    pushq %rax
    popq %rdx
    popq %rcx
    popq %rax
    movb %dl, (%rax,%rcx,1)
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rax, -72(%rbp)
L_loop_top_2:
    movl -72(%rbp), %eax
    pushq %rax
    movabsq $0x8, %rax
    pushq %rax
    popq %rcx
    popq %rax
    cmpq %rcx, %rax
    setb %al
    movzbq %al, %rax
    pushq %rax
    popq %rax
    testq %rax, %rax
    jz L_loop_end_3
    movq -40(%rbp), %rax
    pushq %rax
    movq -48(%rbp), %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    movabsq $0x8, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    movl -72(%rbp), %eax
    pushq %rax
    popq %rax
    pushq %rax
    movabsq $0x7, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -80(%rbp)
    movq -80(%rbp), %rax
    pushq %rax
    movabsq $0x0, %rax
    pushq %rax
    movq -32(%rbp), %rax
    pushq %rax
    movl -72(%rbp), %eax
    pushq %rax
    movabsq $0x7, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    movabsq $0x8, %rax
    pushq %rax
    popq %rcx
    popq %rax
    imulq %rcx, %rax
    movl %eax, %eax
    pushq %rax
    popq %rcx
    popq %rax
    shrq %cl, %rax
    pushq %rax
    movabsq $0xff, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    popq %rax
    movzbq %al, %rax
    pushq %rax
    popq %rdx
    popq %rcx
    popq %rax
    movb %dl, (%rax,%rcx,1)
    movl -72(%rbp), %eax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    movl %eax, %eax
    pushq %rax
    popq %rax
    movq %rax, -72(%rbp)
    movq $0, %rax
    pushq %rax
    popq %rax
    jmp L_loop_top_2
L_loop_end_3:
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    movq $0, %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    .seh_endproc
    .section .iii.ring3,"n"
    .asciz "tp_table_get_fn"
    .text
    .global L_tp_table_get_fn
    .seh_proc L_tp_table_get_fn
L_tp_table_get_fn:
    pushq %rbp
    .seh_pushreg %rbp
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    subq $1024, %rsp
    .seh_stackalloc 1024
    .seh_endprologue
    movq %rcx, -8(%rbp)
    movl -8(%rbp), %eax
    pushq %rax
    movabsq $0x18, %rax
    pushq %rax
    popq %rcx
    popq %rax
    cmpq %rcx, %rax
    setae %al
    movzbq %al, %rax
    pushq %rax
    popq %rax
    testq %rax, %rax
    jz L_if_end_5
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    popq %rax
L_if_end_5:
    leaq L_g_transform_fn_table(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -16(%rbp)
    movl -8(%rbp), %eax
    pushq %rax
    popq %rax
    pushq %rax
    movabsq $0xff, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    movabsq $0x18, %rax
    pushq %rax
    popq %rcx
    popq %rax
    imulq %rcx, %rax
    pushq %rax
    popq %rax
    movq %rax, -24(%rbp)
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rax, -32(%rbp)
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rax, -40(%rbp)
L_loop_top_6:
    movl -40(%rbp), %eax
    pushq %rax
    movabsq $0x8, %rax
    pushq %rax
    popq %rcx
    popq %rax
    cmpq %rcx, %rax
    setb %al
    movzbq %al, %rax
    pushq %rax
    popq %rax
    testq %rax, %rax
    jz L_loop_end_7
    movq -16(%rbp), %rax
    pushq %rax
    movq -24(%rbp), %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    movabsq $0x8, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    movl -40(%rbp), %eax
    pushq %rax
    popq %rax
    pushq %rax
    movabsq $0x7, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -48(%rbp)
    movq -32(%rbp), %rax
    pushq %rax
    movq -48(%rbp), %rax
    pushq %rax
    movabsq $0x0, %rax
    pushq %rax
    popq %rcx
    popq %rax
    movzbq (%rax,%rcx,1), %rax
    pushq %rax
    popq %rax
    pushq %rax
    movabsq $0xff, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    movl -40(%rbp), %eax
    pushq %rax
    popq %rax
    pushq %rax
    movabsq $0x7, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    movabsq $0x8, %rax
    pushq %rax
    popq %rcx
    popq %rax
    imulq %rcx, %rax
    pushq %rax
    popq %rcx
    popq %rax
    shlq %cl, %rax
    pushq %rax
    popq %rcx
    popq %rax
    orq %rcx, %rax
    pushq %rax
    popq %rax
    movq %rax, -32(%rbp)
    movl -40(%rbp), %eax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    movl %eax, %eax
    pushq %rax
    popq %rax
    movq %rax, -40(%rbp)
    movq $0, %rax
    pushq %rax
    popq %rax
    jmp L_loop_top_6
L_loop_end_7:
    movq -32(%rbp), %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    movq $0, %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    .seh_endproc
    .section .iii.ring3,"n"
    .asciz "tp_table_get_src"
    .text
    .global tp_table_get_src
    .seh_proc tp_table_get_src
tp_table_get_src:
    pushq %rbp
    .seh_pushreg %rbp
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    subq $1024, %rsp
    .seh_stackalloc 1024
    .seh_endprologue
    movq %rcx, -8(%rbp)
    movl -8(%rbp), %eax
    pushq %rax
    movabsq $0x18, %rax
    pushq %rax
    popq %rcx
    popq %rax
    cmpq %rcx, %rax
    setae %al
    movzbq %al, %rax
    pushq %rax
    popq %rax
    testq %rax, %rax
    jz L_if_end_9
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    popq %rax
L_if_end_9:
    leaq L_g_transform_fn_table(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -16(%rbp)
    movl -8(%rbp), %eax
    pushq %rax
    popq %rax
    pushq %rax
    movabsq $0xff, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    movabsq $0x18, %rax
    pushq %rax
    popq %rcx
    popq %rax
    imulq %rcx, %rax
    pushq %rax
    popq %rax
    movq %rax, -24(%rbp)
    movq -16(%rbp), %rax
    pushq %rax
    movq -24(%rbp), %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -32(%rbp)
    movq -32(%rbp), %rax
    pushq %rax
    movabsq $0x0, %rax
    pushq %rax
    popq %rcx
    popq %rax
    movzbq (%rax,%rcx,1), %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    movq $0, %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    .seh_endproc
    .section .iii.ring3,"n"
    .asciz "tp_table_get_dst"
    .text
    .global tp_table_get_dst
    .seh_proc tp_table_get_dst
tp_table_get_dst:
    pushq %rbp
    .seh_pushreg %rbp
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    subq $1024, %rsp
    .seh_stackalloc 1024
    .seh_endprologue
    movq %rcx, -8(%rbp)
    movl -8(%rbp), %eax
    pushq %rax
    movabsq $0x18, %rax
    pushq %rax
    popq %rcx
    popq %rax
    cmpq %rcx, %rax
    setae %al
    movzbq %al, %rax
    pushq %rax
    popq %rax
    testq %rax, %rax
    jz L_if_end_11
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    popq %rax
L_if_end_11:
    leaq L_g_transform_fn_table(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -16(%rbp)
    movl -8(%rbp), %eax
    pushq %rax
    popq %rax
    pushq %rax
    movabsq $0xff, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    movabsq $0x18, %rax
    pushq %rax
    popq %rcx
    popq %rax
    imulq %rcx, %rax
    pushq %rax
    popq %rax
    movq %rax, -24(%rbp)
    movq -16(%rbp), %rax
    pushq %rax
    movq -24(%rbp), %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rax
    movq %rax, -32(%rbp)
    movq -32(%rbp), %rax
    pushq %rax
    movabsq $0x0, %rax
    pushq %rax
    popq %rcx
    popq %rax
    movzbq (%rax,%rcx,1), %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    movq $0, %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    .seh_endproc
    .section .iii.ring3,"n"
    .asciz "tp_table_call"
    .text
    .global tp_table_call
    .seh_proc tp_table_call
tp_table_call:
    pushq %rbp
    .seh_pushreg %rbp
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    subq $1024, %rsp
    .seh_stackalloc 1024
    .seh_endprologue
    movq %rcx, -8(%rbp)
    movq %rdx, -16(%rbp)
    movq %r8, -24(%rbp)
    movq %r9, -32(%rbp)
    movl -8(%rbp), %eax
    pushq %rax
    popq %rcx
    subq $32, %rsp
    callq L_tp_table_get_fn
    addq $32, %rsp
    pushq %rax
    popq %rax
    movq %rax, -40(%rbp)
    movq -40(%rbp), %rax
    pushq %rax
    movabsq $0x0, %rax
    pushq %rax
    popq %rcx
    popq %rax
    cmpq %rcx, %rax
    sete %al
    movzbq %al, %rax
    pushq %rax
    popq %rax
    testq %rax, %rax
    jz L_if_end_13
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    popq %rax
L_if_end_13:
    movabsq $0x100000, %rax
    pushq %rax
    movq -32(%rbp), %rax
    pushq %rax
    movq -24(%rbp), %rax
    pushq %rax
    movq -16(%rbp), %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    movq -40(%rbp), %rax
    pushq %rax
    popq %rax
    callq *%rax
    addq $32, %rsp
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    movq $0, %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    .seh_endproc
    .section .iii.ring3,"n"
    .asciz "_tx_register"
    .text
    .global L__tx_register
    .seh_proc L__tx_register
L__tx_register:
    pushq %rbp
    .seh_pushreg %rbp
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    subq $1024, %rsp
    .seh_stackalloc 1024
    .seh_endprologue
    movq %rcx, -8(%rbp)
    movq %rdx, -16(%rbp)
    movq %r8, -24(%rbp)
    movq %r9, -32(%rbp)
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    subq $32, %rsp
    callq pattern_template_zero
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    leaq L_TX_NAME_BUF(%rip), %rax
    pushq %rax
    movabsq $0x0, %rax
    pushq %rax
    movabsq $0x74, %rax
    pushq %rax
    popq %rdx
    popq %rcx
    popq %rax
    movb %dl, (%rax,%rcx,1)
    leaq L_TX_NAME_BUF(%rip), %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movl -8(%rbp), %eax
    pushq %rax
    movabsq $0xa, %rax
    pushq %rax
    popq %rcx
    popq %rax
    xorl %edx, %edx
    divq %rcx
    pushq %rax
    movabsq $0xff, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    popq %rax
    movzbq %al, %rax
    pushq %rax
    movabsq $0x30, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    popq %rdx
    popq %rcx
    popq %rax
    movb %dl, (%rax,%rcx,1)
    leaq L_TX_NAME_BUF(%rip), %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    movl -8(%rbp), %eax
    pushq %rax
    movabsq $0xa, %rax
    pushq %rax
    popq %rcx
    popq %rax
    xorl %edx, %edx
    divq %rcx
    movq %rdx, %rax
    pushq %rax
    movabsq $0xff, %rax
    pushq %rax
    popq %rcx
    popq %rax
    andq %rcx, %rax
    pushq %rax
    popq %rax
    movzbq %al, %rax
    pushq %rax
    movabsq $0x30, %rax
    pushq %rax
    popq %rcx
    popq %rax
    addq %rcx, %rax
    pushq %rax
    popq %rdx
    popq %rcx
    popq %rax
    movb %dl, (%rax,%rcx,1)
    leaq L_TX_NAME_BUF(%rip), %rax
    pushq %rax
    movabsq $0x3, %rax
    pushq %rax
    movabsq $0x0, %rax
    pushq %rax
    popq %rdx
    popq %rcx
    popq %rax
    movb %dl, (%rax,%rcx,1)
    movabsq $0x3, %rax
    pushq %rax
    leaq L_TX_NAME_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    leaq L_TX_MODULE_MHASH(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq pattern_template_set_id
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movabsq $0x35a4e900, %rax
    pushq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    popq %rdx
    subq $32, %rsp
    callq pattern_template_set_activation_base
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movabsq $0x6, %rax
    pushq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    popq %rdx
    subq $32, %rsp
    callq pattern_template_set_hexad_kind
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movabsq $0x2, %rax
    pushq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    popq %rdx
    subq $32, %rsp
    callq pattern_template_set_ring
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movabsq $0x389fd980, %rax
    pushq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    popq %rdx
    subq $32, %rsp
    callq pattern_template_set_k_value
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movabsq $0xc, %rax
    pushq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    popq %rdx
    subq $32, %rsp
    callq pattern_template_set_specialisation_of
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -32(%rbp), %rax
    pushq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    popq %rdx
    subq $32, %rsp
    callq pattern_template_set_dispatch
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movzbq -16(%rbp), %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    subq $32, %rsp
    callq pattern_template_set_binding_kind
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movzbq -24(%rbp), %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    subq $32, %rsp
    callq pattern_template_set_binding_kind
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    leaq L_TX_TEMPLATE_BUF(%rip), %rax
    pushq %rax
    popq %rax
    pushq %rax
    movl -8(%rbp), %eax
    pushq %rax
    popq %rcx
    popq %rdx
    subq $32, %rsp
    callq pattern_register
    addq $32, %rsp
    movzbq %al, %rax
    pushq %rax
    popq %rax
    movq %rax, -40(%rbp)
    movzbq -40(%rbp), %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    popq %rcx
    popq %rax
    cmpq %rcx, %rax
    setne %al
    movzbq %al, %rax
    pushq %rax
    popq %rax
    testq %rax, %rax
    jz L_if_end_15
    movabsq $0x1, %rax
    pushq %rax
    popq %rax
    movb %al, L_TX_REG_FAIL(%rip)
    movq $0, %rax
    pushq %rax
    popq %rax
L_if_end_15:
    movq -32(%rbp), %rax
    pushq %rax
    movzbq -24(%rbp), %rax
    pushq %rax
    movzbq -16(%rbp), %rax
    pushq %rax
    movl -8(%rbp), %eax
    pushq %rax
    movabsq $0x12, %rax
    pushq %rax
    popq %rcx
    popq %rax
    subq %rcx, %rax
    movl %eax, %eax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L_tp_table_set
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    movq $0, %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    .seh_endproc
    .section .iii.ring3,"n"
    .asciz "transform_register_all"
    .text
    .global transform_register_all
    .seh_proc transform_register_all
transform_register_all:
    pushq %rbp
    .seh_pushreg %rbp
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    subq $1024, %rsp
    .seh_stackalloc 1024
    .seh_endprologue
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movb %al, L_TX_REG_FAIL(%rip)
    leaq tp_iii_to_asm(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -8(%rbp)
    leaq tp_iii_to_babel_json(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -16(%rbp)
    leaq tp_babel_json_to_iii(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -24(%rbp)
    leaq tp_asm_to_pe(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -32(%rbp)
    leaq tp_ast_bin_to_iii(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -40(%rbp)
    leaq tp_babel_cbor_json(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -48(%rbp)
    leaq tp_babel_json_cbor(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -56(%rbp)
    leaq tp_babel_text(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -64(%rbp)
    leaq tp_ripple_dot(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -72(%rbp)
    leaq tp_iii_to_ast_bin(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -80(%rbp)
    leaq tp_iii_to_c99(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -88(%rbp)
    leaq tp_iii_to_latex(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -96(%rbp)
    leaq tp_x86_disasm(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -104(%rbp)
    leaq tp_pe_hex(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -112(%rbp)
    leaq tp_ripple_md(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -120(%rbp)
    leaq tp_ast_dot(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -128(%rbp)
    leaq tp_babel_text_back(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -136(%rbp)
    leaq tp_iii_hex(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -144(%rbp)
    leaq tp_c99hdr_to_iii(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -152(%rbp)
    leaq tp_iii_to_md(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -160(%rbp)
    leaq tp_x86_assemble(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -168(%rbp)
    leaq tp_raw_hex(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -176(%rbp)
    leaq tp_ast_to_babel_json(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -184(%rbp)
    leaq tp_babel_json_to_ast(%rip), %rax
    pushq %rax
    popq %rax
    movq %rax, -192(%rbp)
    movq -8(%rbp), %rax
    pushq %rax
    movabsq $0x3, %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x12, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -16(%rbp), %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x13, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -24(%rbp), %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    movabsq $0x14, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -32(%rbp), %rax
    pushq %rax
    movabsq $0x5, %rax
    pushq %rax
    movabsq $0x3, %rax
    pushq %rax
    movabsq $0x15, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -40(%rbp), %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x7, %rax
    pushq %rax
    movabsq $0x16, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -48(%rbp), %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    movabsq $0x8, %rax
    pushq %rax
    movabsq $0x17, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -56(%rbp), %rax
    pushq %rax
    movabsq $0x8, %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    movabsq $0x18, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -64(%rbp), %rax
    pushq %rax
    movabsq $0x9, %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    movabsq $0x19, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -72(%rbp), %rax
    pushq %rax
    movabsq $0xd, %rax
    pushq %rax
    movabsq $0xe, %rax
    pushq %rax
    movabsq $0x1a, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -80(%rbp), %rax
    pushq %rax
    movabsq $0x7, %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x1b, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -88(%rbp), %rax
    pushq %rax
    movabsq $0x4, %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x1c, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -96(%rbp), %rax
    pushq %rax
    movabsq $0xa, %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x1d, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -104(%rbp), %rax
    pushq %rax
    movabsq $0x3, %rax
    pushq %rax
    movabsq $0xf, %rax
    pushq %rax
    movabsq $0x1e, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -112(%rbp), %rax
    pushq %rax
    movabsq $0xc, %rax
    pushq %rax
    movabsq $0x5, %rax
    pushq %rax
    movabsq $0x1f, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -120(%rbp), %rax
    pushq %rax
    movabsq $0xb, %rax
    pushq %rax
    movabsq $0xe, %rax
    pushq %rax
    movabsq $0x20, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -128(%rbp), %rax
    pushq %rax
    movabsq $0xd, %rax
    pushq %rax
    movabsq $0x7, %rax
    pushq %rax
    movabsq $0x21, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -136(%rbp), %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    movabsq $0x9, %rax
    pushq %rax
    movabsq $0x22, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -144(%rbp), %rax
    pushq %rax
    movabsq $0xc, %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x23, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -152(%rbp), %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x10, %rax
    pushq %rax
    movabsq $0x24, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -160(%rbp), %rax
    pushq %rax
    movabsq $0xb, %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    movabsq $0x25, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -168(%rbp), %rax
    pushq %rax
    movabsq $0xf, %rax
    pushq %rax
    movabsq $0x3, %rax
    pushq %rax
    movabsq $0x26, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -176(%rbp), %rax
    pushq %rax
    movabsq $0xc, %rax
    pushq %rax
    movabsq $0x11, %rax
    pushq %rax
    movabsq $0x27, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -184(%rbp), %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    movabsq $0x7, %rax
    pushq %rax
    movabsq $0x28, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movq -192(%rbp), %rax
    pushq %rax
    movabsq $0x7, %rax
    pushq %rax
    movabsq $0x2, %rax
    pushq %rax
    movabsq $0x29, %rax
    pushq %rax
    popq %rcx
    popq %rdx
    popq %r8
    popq %r9
    subq $32, %rsp
    callq L__tx_register
    addq $32, %rsp
    movslq %eax, %rax
    pushq %rax
    popq %rax
    movzbq L_TX_REG_FAIL(%rip), %rax
    pushq %rax
    movabsq $0x1, %rax
    pushq %rax
    popq %rcx
    popq %rax
    cmpq %rcx, %rax
    sete %al
    movzbq %al, %rax
    pushq %rax
    popq %rax
    testq %rax, %rax
    jz L_if_end_17
    movabsq $0x1, %rax
    pushq %rax
    popq %rax
    negq %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    popq %rax
L_if_end_17:
    movabsq $0x0, %rax
    pushq %rax
    popq %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    movq $0, %rax
    pushq %rax
    movq $0, %rax
    movq %rbp, %rsp
    popq %rbp
    retq
    .seh_endproc
