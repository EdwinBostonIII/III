...
declare -A EXPECTED=(
    ...
    [1862_grail_characteristic_firewall]=99 # GRAIL L8 BROADER-REACH: generalizes M1 (logic<->crypto). The char-2 -> char-q obstruction is GENERIC, not a 3329 coincidence: both obstructions (no unital ring hom char-2->Z_m since 2!=0/gcd(2,m)=1; only additive idempotent is 0) hold for EVERY deployed modulus -- 3329(ML-KEM), 12289(Falcon), 8380417(ML-DSA), 2^31-1, 2^61-1, Goldilocks 2^64-2^32+1(STARK), 3233=61*53(RSA composite). A structural characteristic firewall. Scope (as M1): canonical morphism classes only; simulation/circuits unaffected.
