...
declare -A EXPECTED=(
    ...
    [1841_grail_vj_strategy_improvement]=99 # GRAIL L8 attack: the COMPLETE solver -- VOGE-JURDZINSKI discrete play-profile valuation (anchor,set,len) + Odd best-response SI. On the SAME 80 games as 1840: matches the Zielonka oracle 80/80 (greedy was 77/80) -- the LOCAL OPTIMA are removed, exactly as VJ proves. Teeth-checked: SI-disabled (cmp==0) -> 40/80, parity-flipped -> 0/80, so the 80/80 is non-vacuous. Open question deferred: improvement-step counts (where Friedmann's exp lower bounds live).
