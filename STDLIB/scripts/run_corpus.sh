...
declare -A EXPECTED=(
    ...
    [1859_grail_partial_solver_residue]=99 # GRAIL L8 RESIDUAL-HOPE (B): "push sound partial solvers to 100%" dies on an Omega(n) lower bound. On the n-cycle the winner is GLOBAL (one far max-priority vertex) and there is NO proper closed subset, so no bounded-width dominion exists -> a width-<n dominion solver decides 0 of n while the oracle decides all; residue=n grows with the game. Locality cannot see a global cycle-max; completing needs the full recursion.
