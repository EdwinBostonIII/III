...
declare -A EXPECTED=(
    ...
    [1882_parity_game_faculty]=99 # WALL#1 CASHED IN: gates the PRODUCTION numera::parity_game faculty (the parity-game / modal-mu-calculus verification ENGINE -- Zielonka attractor decomposition, @export pg_reset/set_owner/set_prio/add_edge/solve_even/winner/kat, <=64-vertex u64-bitmask arenas, registered in build_stdlib + archived in libiii_native.a). pg_winner == an INDEPENDENT brute-force game oracle (enumerate ALL positional strategies of both players) on every vertex of 2000 random games + pg_kat()==99 (control-blindness witness + deadend sentinel + self-loop).
    [1883_sat_tractable_faculty]=99 # WALL#2 CASHED IN: gates the PRODUCTION numera::sat_tractable faculty (the Schaefer-class POLY SAT fast-path the CDCL solver numera::sat lacks). @export CNF mode (st_reset/st_clause/st_classify/st_decide -> SAT/UNSAT/NOT_TRACTABLE over 0-valid/1-valid/Horn/dual-Horn/2-SAT) + XOR/affine mode (stx_reset/stx_eq/stx_decide, Gaussian over GF(2)); registered + archived in libiii_native.a. Every class decider == brute-force 2^n oracle on thousands of random instances (both SAT/UNSAT) + non-tractable correctly flagged + st_kat()==99.
    [1884_confluence_faculty]=99 # WALL#3 CASHED IN: gates the PRODUCTION numera::confluence faculty (a Newman-based confluence DECIDER for finite abstract rewriting systems -- verify any rule set/normalisation/merge relation is confluent at runtime). @export cf_reset/cf_step/cf_terminating/cf_locally_confluent/cf_confluent/cf_count_nf/cf_unique_nf/cf_kat; archived in libiii_native.a. On random terminating ARS the 3 Newman-equivalent computations (direct global confluence / local confluence / unique normal forms) all agree; + the non-terminating boundary witness (1869) in cf_kat. Decidable finite-state ISLAND (general TRS confluence undecidable).
    [1885_graph_refine_faculty]=99 # WALL#4 CASHED IN: gates the PRODUCTION numera::graph_refine faculty (graph fingerprint + EXACT isomorphism, for systems-graph dedup). @export gr_reset_a/b, gr_edge_a/b, gr_fingerprint_a (1-WL stable-colouring hash = sound poly invariant), gr_ncolours_a, gr_iso (1-WL filter + colour-pruned backtracking = sound+complete); archived in libiii_native.a (internals GR_-prefixed for harmony). gr_iso == brute permutation-iso oracle on 3000 random pairs (both outcomes); gr_kat covers the K33/prism 1-WL blind spot (gr_iso still correct).
    [1886_diophantine_faculty]=99 # WALL#6/11 ISLAND CASHED IN: gates the PRODUCTION numera::diophantine faculty (linear Diophantine solver + extended Euclid + modular inverse -- the decidable island of Hilbert 10). @export dio_gcd / dio_ext_gcd (+dio_bezout_x/y) / dio_solve(a,b,c)->solvable + dio_sol_x/y / dio_mod_inverse(a,m); archived. dio_solve == bounded brute on 4000 random equations (both outcomes), each solution satisfies the eqn, a*x+b*y=gcd identity, and a*inv==1 mod m for 4000 modular inverses.
    [1887_con_lattice_faculty]=99 # WALL#5 CASHED IN: gates the PRODUCTION numera::con_lattice faculty (congruence lattice of a finite one-binary-operation algebra -- the decidable side of FLRP). @export cl_reset/cl_op/cl_num_congruences/cl_is_distributive/cl_kat; archived. cl_kat: Con(Klein-4 V4)=M3 (5 congruences, NON-distributive, the 1873 island) + Con(Z3)=2-chain + Con(Z4)=3-chain (distributive). Gate: cl_num_congruences == an INDEPENDENT in-KAT congruence enumeration on 3000 random binary algebras (n=3..5).
    [1888_constructible_faculty]=99 # WALL#9 CASHED IN: gates the PRODUCTION numera::constructible faculty (Wantzel constructibility + rational-root cubic irreducibility + Gauss-Wantzel n-gon). @export ctr_has_rational_root / ctr_cubic_irreducible / ctr_is_pow2 / ctr_constructible_from_degree / ctr_ngon_constructible / ctr_kat; archived. ctr_kat: doubling-cube + trisection impossible (x^3-2, 8x^3-6x-1 irreducible deg3, the 1879 island). Gate: ctr_has_rational_root == brute root-search on 4000 random cubics + ctr_ngon_constructible == independent Gauss-Wantzel for n=3..400.
    [1889_ramsey_faculty]=99 # WALL#8 CASHED IN: gates the PRODUCTION numera::ramsey faculty (monochromatic-clique detection in a 2-coloured graph + small Ramsey numbers). @export ram_reset/ram_set_color/ram_has_mono_clique(k)/ram_ramsey_33()->6/ram_kat; archived (prefix ram_/RAM_ -- rm_ collided with forcefield/ripple_metric). ram_ramsey_33 computes R(3,3)=6 by exhaustion (K5 pentagon triangle-free + all 2^15 K6 colourings have a mono triangle). Gate: ram_has_mono_clique == an independent in-KAT subset check on 3000 random colourings.
    [1890_comm_lb_faculty]=99 # WALL#7 CASHED IN: gates the PRODUCTION numera::comm_lb faculty (communication-complexity lower bound by the rank method). @export cml_reset/cml_set/cml_rank_gf2/cml_lb (= ceil(log2 rank) = a PROVEN D(f) lower bound, the climbed face of the comm-complexity wall) / cml_kat; archived. cml_kat: EQ 8x8 identity -> rank 8, D(EQ)>=3; constant -> rank 1, lb 0. Gate: cml_rank_gf2 == an independent GF(2) elimination on 4000 random matrices + cml_lb == ceil(log2 rank).
    [1891_goodstein_faculty]=99 # WALL#10 CASHED IN: gates the PRODUCTION numera::goodstein faculty (Goodstein sequences, hereditary base-bump-and-subtract, bounded termination -- the island of the INDEPENDENCE wall). @export gst_terminates(m,maxsteps)->REACHED/OVERFLOW/CAP, gst_step_count, gst_kat; archived. gst_kat: G(1),G(2),G(3) terminate; G(16)=2^(2^2) overflows u64 in ~2 steps (unbounded growth = the computational shadow of PA-independence, Kirby-Paris). G(4) within 200 steps still going (cap).
