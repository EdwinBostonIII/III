...
declare -A EXPECTED=(
    ...
    [2613_mathesis_grammar]=99          # Ξ5/P1 THE GRAMMAR RATCHET TEACH: e-graph learns AND/OR/XOR/SHR, every rule a sealed library schema (0001/0006/0007/tier-1/2601 ranges); mul/div plan FNV sweep PIN unmoved; false-identity + range teeth; full two-path shift-chain sweep (3906 disposer calls); congruence + determinism (~60s)
    [2614_disposer_early_return]=99     # THE EARLY-RETURN SOUNDNESS GATE (found by pointing COMPILED/iii-prove at ordinary user code): sd_denote's RET used to return the top of stack IMMEDIATELY, so `if c {return b} return a` denoted as **b alone** — a FALSE PROVEN (`if c{1}else 2` ≡ `if c{1}else 3`), the one unrecoverable outcome. The false-PROVEN arm is REPRODUCED first (RED=10), then the path-condition accumulator fixes it; + semantic truth (branchy ≡ branchless select), guard-ladder precedence, CHECKED totality (fall-off-end REFUSED), no-regression (fast path + if/else mux), and the DIV_U/REM_U-by-const-pow2 fragment (align/mask idioms decidable; non-pow2 + symbolic divisor REFUSED; x/0≡0 exact)
    [2611_mathesis_novel]=99            # Ξ1-T2 THE NOVELTY+DEDUP ORGAN (R3 COMPUTED): two-grain dedup (structural canon + judge-confirmed semantic classes: cancellation/involution = ONE equation); Ξ0 schema machine-detected as rediscovery via taught-e-graph derivability; ADD-chain = the knowledge boundary (novel); mul ⇒ honest abstain; determinism
    [2612_mathesis_frontier]=99         # Ξ1-T3/R7 THE FRONTIER QUEUE: blockers named+counted; THE RETRY LIVE — mul-assoc PROVEN over ALL Z/2^8 (complete 2^24 exhaustion, SAT wall measured + routed around) + R1 dual (sampled instances symbolic); false pair REFUTED; w=64 wall stays queued; bounds+determinism
    [2674_mathesis_order]=99            # Ξ9-T1+T3 THE STATEMENT LATTICE COMPLETED: R4-order (∀x:x≥s0 REFUTED) + false-converse + false-collapse arms first; reflexivity/meet-lower-bound/antisymmetry/transitivity/AND-monotonicity + signed floor/ceiling via the compare fragment; THE FIRST CLOSED-OPTIMAL CERTIFICATES — cost(align_k)=cost(mask_k)=EXACTLY 1 ∀k∈1..63 (1-op upper + id-refute + witness-function const-exclusion per k, 378 symbolic calls); rot-2op → frontier blocker 3
    [2676_mathesis_derive]=99           # Ξ10-T1 THE DEDUCTION ORGAN integrity: step-gap UNTYPEABLE (CIC kernel refuses); tampered premise ref REJECTED (id re-derived from canonical descriptor); wrong conclusion REFUTED at every ground n (two-path); MX04 descriptor law (malformed-no-id, deterministic, statement-sensitive); the honest rule-1 derivation ADMITS
    [2677_mathesis_induct]=99           # Ξ10-T2 THE FIRST UNBOUNDED QUANTIFIERS: ∀n≥2 n-fold AND-chain collapse (0001 as base+step) + ∀n∈ℕ rot_1ⁿ≡rot_{n mod 64} (the 64-residue Cayley row re-discharged + symbolic towers n=2..6 + native to 200); THE FALSE INDUCTION (mod 63) DIES at its ground leg; 0002-ref cross-derived via mxd_ref_of (LE trap gated)
    [2678_mathesis_orbit]=99            # Ξ11/P9 ROT-CONJUGATION ORBITS: the equivariance table MACHINE-PROVEN both halves — AND/OR/XOR × k=1..63 equivariant (189 calls over all 2^128 pairs); ADD/SUB/MUL/SHL/SHR REFUTED at k=1 (transport refusal is algebra, not policy); MACHINE entries 0006/0007 orbit instances re-witnessed at k∈{1,17,32,63}
    [2679_mathesis_width]=99            # Ξ11 WIDTH FUNCTORS: transport = re-proof, both directions — the (x<<32)>>32≢x tooth pinned; mask-low k=32 TRUE@64 but its naive w=32 transport REFUTED (count≥w refusal grounded); x<<8≡0 TRUE@8 REFUTED@64 (up-refusal); the AND-chain width family re-proven at 32/16/8 (R2)
    [2640_mathesis_certify]=99          # Ξ4-T1 THE KERNEL CERTIFICATION LEG: every sealed equation cross-judged by the CIC kernel's BV64 iota-fold (the third meaning-bearer) at 8 semantic-edge vectors — 0001/0006/0007/MLB/mul-assoc@8/n=4 citizen/C64 Cayley instance/align_k; kernel-vs-SVIR shift-semantics boundary honest (counts<64 only); refusal arms (wrong fold, wrong absorption, ill-typed = NO type)
    [2641_mathesis_library]=99          # Ξ4-T2 THE LIBRARY IS PUBLISHED: dedup-by-content-address (same descriptor → one id, idempotent); statement-sensitivity (any field differs → distinct id, MX01+MX03); THE PROVENANCE TALLY (exactly 6 MACHINE entries of 18 turn the discovery ratchet); cold queryability (every id re-derives from its descriptor, stateless)
    [2650_mathesis_loop]=99             # Ξ5/Ξ13 THE STANDING ROUNDS: round-2 shift tier — the whole declared 8064-instance space disposed per-instance (S1 honoured), chains/align/mask REDISCOVERED (e-graph-derivable ⇒ refused), SATURATION-ANNIHILATION 2016+2016 = THE MACHINE HARVEST (in no rule set ⇒ novel); K=2 dry convergence (second sweep byte-identical); H10 no-revert inside the loop (~90s)
    [2651_mathesis_ratchet]=99          # Ξ5-T2/H10 THE RATCHET IN EXECUTABLE FORM (DOCS/MATHESIS-RATCHET.txt): teach-gap stays 0 (4 ops re-derivable); both unbounded citizens re-derive (5 legs); CLOSED-OPTIMAL stands at k∈{1,32,63}; frontier retry stands (mul-assoc@8); the door stays strict — ANY revert reddens
    [2682_mathesis_autonomy]=99         # Ξ13/P10 THE AUTONOMY INVARIANT (in-process half): ONE binary, constants only — genesis→0001→head COLD replay against the 2603 pin (flipped-byte tooth); a complete discover→classify→discriminate cycle with no subprocess/operator/file; deterministic re-run
    [2683_mathesis_agenda]=99           # Ξ13 MEASURED INTENT: unmeasured item REFUSED; live census (seeded body → 2 windows) + live frontier counts feed the agenda; ordering law (value desc, kind, ident) deterministic; fingerprint reproducible across insertion orders, forged measurement ≠ fingerprint
...
for src in "$CORPUS_DIR"/[0-9][0-9]_*.iii "$CORPUS_DIR"/[0-9][0-9][0-9]_*.iii "$CORPUS_DIR"/[0-9][0-9][0-9][0-9]_*.iii; do
    ...
    case "$base" in
        ...
        2104_field_kolmogorov|2105_field_color|2106_field_time|2107_field_inverse|2108_field_slice|2109_field_quantum|2110_field_acoustic|2111_field_reversible|2112_field_localweb|2113_field_selfpop|2114_field_cf|2115_field_wave|2116_field_hash|2117_field_superpos|2118_field_binding)
            ...
            SKIP=$((SKIP+1)); continue
            ;;
        2120_bigint_isqrt|2121_sqrt_sum_sign|2122_lazy_real|2123_lazy3|2124_transcendental|2125_verb_geom|2137_adaptive_sign|2138_symmetry_quotient|2139_padic_barrier|2140_adaptive_big|2141_cyclotomic_rotation|2142_se3_screw|2143_traj_arclen|2144_lattice_pathfind|2145_denest|2146_compactor|2147_lattice_oracle|2148_theorem_fuzzer|2149_universal_block|2151_photon_route|2152_mechanism|2153_collision|2156_sturm|2157_algnum|2159_kf_weld|2680_mathesis_denest|2681_mathesis_envelope)
            RESULTS+=("SKIP  $base : sqrt-sum sign / lazy real / verb-geom / photon route / mechanism / collision / sturm / algebraic-numbers / mathesis-telescope 2680+2681 (Xi12; also exercised by the mathesis engine harness) -- heart-adopted 2026-07-17 (ex run_sqrtsum_kats, retired; protocol: links sqrt_sum_sign + exact_denest + traj_kinematics + cyclotomic_se3 + collide + sturm + algnum + bigint)")
            SKIP=$((SKIP+1)); continue
            ;;
        ...
        2700_mathesis_alg_judge|2701_mathesis_denest_general|2702_mathesis_cube_ramanujan|2703_mathesis_mx04_chain|2704_ripple_spectral|2705_mathesis_ring_judge|2706_mathesis_ring_drain|2707_mathesis_curve_hunt|2708_mathesis_group_tau|2709_mathesis_forge|2710_mathesis_pilot|2711_mathesis_ontogenesis|2712_mathesis_rot2|2713_mathesis_bigcurve|2714_mathesis_norm|2715_mathesis_pilot12|2716_mathesis_tetra|2717_mathesis_bigjudge|2718_mathesis_tetra_full|2719_mathesis_pattern|2720_mathesis_pilot18|2721_mathesis_orbits|2722_mathesis_box50|2723_mathesis_census|2724_mathesis_rot64|2725_mathesis_pilot24|2726_mathesis_rot64_omega|2727_mathesis_rot64_omega2|2728_mathesis_census10|2729_mathesis_pattern_omega2|2730_mathesis_pilot30|2731_mathesis_rot64_universal1|2732_mathesis_rot64_universal2|2733_mathesis_rot64_universal3|2734_mathesis_census11|2735_mathesis_pilot36|2736_mathesis_op3_census|2737_mathesis_op3_cert_rot|2738_mathesis_op3_cert_neg|2739_mathesis_pilot42|2740_mathesis_op3_sweep|2741_mathesis_op3_drain_cert_rot|2742_mathesis_op3_drain_cert_neg|2743_mathesis_op3_box_cert|2744_mathesis_op3_sweep2|2745_sigma_floor_census|2746_sha512_core_optimum|2747_sha512_diffusion_break)
            ...
            ;;
        2762_conservation)
            RESULTS+=("SKIP  $base : THE CONSERVATION TIER (III decides information conservation autonomously on both sides -- reversible WORDS of its die [aether/reversibility: GF(2) autonomous inversion + computational diodes] + the NONLINEAR inverse it could not GF(2)-invert [MUL-odd via III's own Newton iteration, round-tripped] + reversible ONTOLOGIES of the fair magma universe [aether/xeno_ontogenesis conservation lens: quasigroup/Latin-square census by exhaustion]) -- heart-tracked C2 + derived live by the ERGON deep verb (iii-ergon deep, ex run_standing_tools; links reversibility.o + xeno_ontogenesis.o + substrate_ontogenesis.o + isa_ontogenesis.o + bv_bits.o; stored-answer-free: census re-computed, inverse re-synthesized, round-tripped, negative arm)")
            SKIP=$((SKIP+1)); continue
            ;;
        2763_autophasis|2764_synapse)
            RESULTS+=("SKIP  $base : THE AUTOPHASIS SELF-UTTERANCE + THE SYNAPSE WIRE (the organism presses its discovery into a NATIVE ORGAN -- press-deterministic, sealed-id pinned, byte-identical, tampered utterance REFUSED; and two endpoints prove the same being over a real socket under the autognosis constitution root, foreign constitutions refused BY NAME) -- heart-tracked C2 + derived live by the ERGON deep verb + introspection seat (iii-ergon; ex run_standing_tools; links autophasis.o + xeno_ontogenesis.o + substrate_ontogenesis.o / synapse.o + autognosis.o + autophasis.o + cg_glossa_rules.o + cg_phys_rules.o)")
            SKIP=$((SKIP+1)); continue
            ;;
        2765_anastasis)
            RESULTS+=("SKIP  $base : THE ANASTASIS TIER (every fallen op REDEEMED -- the minimal lift pays the EXACT measured information deficit, proven injective + reconstructed over all 2^13; Bennett involution proven per op at three widths; the redeemed lifts form a PROVEN abelian group; stripped-keep negative teeth) -- heart-tracked C2 + derived live by the ERGON deep verb (iii-ergon deep; ex run_standing_tools redeem; links reversibility.o + isa_ontogenesis.o + cpufeat.o + bv_bits.o + sat.o)")
            SKIP=$((SKIP+1)); continue
            ;;
        2767_lagrangian)
            RESULTS+=("SKIP  $base : THE LAGRANGIAN COUPLING (two III nodes fold ONE reversible admix chain -- state never on the wire, only Action Vectors; entanglement by root exchange; latency as a recoverable Landauer deficit; bounded least-action superposition -> provisional collapse -> EXACT reversal via the proven bijection; the Gestalt converges bit-exact or a bit is lost to the void) -- heart-tracked C2 + derived live by the ERGON deep verb (iii-ergon deep couple; ex run_standing_tools; links lagrangian.o + admix.o + autognosis.o + autophasis.o + xeno_ontogenesis.o + substrate_ontogenesis.o + cg_glossa_rules.o + cg_phys_rules.o + glossa.o)")
            SKIP=$((SKIP+1)); continue
            ;;
        2770_crucible)
            RESULTS+=("SKIP  $base : THE CRUCIBLE (morphogenetic evolution with a cryptographic floor -- III mutates its own physics under stress, BEAM-SEARCHES worthy mutations [a cheap O(1)-validity + 2-point-avalanche proxy ranks a generation; only the top-k pay the EXPENSIVE round-trip-bijectivity + 16-point-avalanche + kardia self-proof verdict], and adopts ONLY a survivor proven flawless; the live seal is never overwritten).  MEASURED: beam finds a flawless survivor in ~4 expensive verdicts where blind mutation spends ~nvalid -- evolution costs LESS, not more (the user's hard constraint, gated).  heart-tracked C2 + derived live by the ERGON deep verb (iii-ergon deep evolve; cr_selfprove is the condition of motion, live seal never overwritten); links crucible.o + kardia.o + cassandra.o + lagrangian.o + admix.o + autognosis.o + autophasis.o + xeno_ontogenesis.o + substrate_ontogenesis.o + cg_glossa_rules.o + cg_phys_rules.o + glossa.o + idfold.o + sat_arith.o + sha256.o + cad.o + mhash.o")
            SKIP=$((SKIP+1)); continue
            ;;
        2768_cassandra|2769_topos)
            RESULTS+=("SKIP  $base : CASSANDRA (thermodynamic branch prediction: 2^K candidate timelines held in K orthogonal color-tags -- derive-don't-store, collapse by thermodynamic filter at ZERO rewind, orthogonality caps the window at 64) + TOPOS (data transmission as GF(2) geometry: a payload projects to a manifold + colors, is predicted in cassandra's superposition before it arrives, and reifies BIT-FOR-BIT with no parser -- serialization eradicated).  Each organ SELF-PROVES as a condition of motion (cass_selfproof / tp_selfprove); the substrate `project` verb is the live consumer.  heart-tracked C2 + derived live by the ERGON deep verb (iii-ergon deep project; cass_selfproof/tp_selfprove are the condition of motion); links cassandra.o + topos.o + lagrangian.o + admix.o + autognosis.o + autophasis.o + xeno_ontogenesis.o + substrate_ontogenesis.o + cg_glossa_rules.o + cg_phys_rules.o + glossa.o + idfold.o")
            SKIP=$((SKIP+1)); continue
            ;;
        2748_nomos_rules|2749_nomothesis_gap|2756_taxis_quotient|2757_oneiros_collapse|2758_nomos_consent|2759_glossa_words)
            RESULTS+=("SKIP  $base : THE NOMOS LEGISLATURE + THE TAXIS ONTOLOGY + THE ONEIROS TIER (machine-legislated compiler law: the sealed-rules ledger gate + THE OWED-LAW ENUMERATOR -- gap census, critical pairs, count-atom candidates + the die's semantic quotient = the objects of the friction order + the dream crucible: owed laws BELIEVED then collapsed to two-path witnesses by algebra-generated points, unoriented joinability docket, seraphic womb, replayable jitter seed) -- heart-tracked C2 + derived live by the ERGON deep verb (iii-ergon deep nomos/oneiros; ex run_standing_tools; links cg_phys_rules.o + isa_ontogenesis.o + oneiros.o + isa_friction_judge.o + friction.o)")
            SKIP=$((SKIP+1)); continue
            ;;
