...
declare -A EXPECTED=(
    ...
    [1855_grail_meanpayoff_weight_obstruction]=99 # GRAIL L8 FORMAL: the QUANT-GAMES face obstruction (PROVEN). The reduction parity->mean-payoff/energy forces priority-separating weights whose MINIMAL magnitudes grow GEOMETRICALLY: w(0)=1, w(p)=1+n*sum_{q<p}w(q) = (n+1)*w(p-1) = (n+1)^p. So the top weight is EXPONENTIAL in d, mean-payoff values need Theta(d log n) bits, and value iteration is only PSEUDO-poly. This is the precise reason the energy/LP/discounted/tropical Class-II survivors inherit the difficulty: parity DOES reduce to mean-payoff, but the numbers are where the alternation hides.
    ...
    [1852_grail_control_blindness_quantitative]=99 # GRAIL L8 FORMAL: theorem T2' (quantitative control-blindness, VERIFIED). A control-free graph invariant (max reachable-cycle priority) has per-vertex agreement with the true winner that DECAYS toward 1/2 as games grow: 82% (n=4) -> 70% (n=10) -> 65% (n=14), monotone. T2 says control-free is wrong on SOME witness; T2' says it is asymptotically UNINFORMATIVE on the whole distribution -- no graph invariant is even approximately right at scale (matches the workflow's "signed zeta -> ~50%").
    ...
    [1851_grail_metagrail_obstruction]=99 # GRAIL L8 FORMAL: theorem M1 (proven-NEGATIVE summit). No unifying invariant across the lattice logics (Boolean-ring face, char 2) and PQ ring Z_3329 (char 3329). Verified in Z_3329: (A) characteristic obstruction -- 2 != 0 and gcd(2,3329)=1 (3329 prime) => no char-2 -> Z_q unital ring hom; (B) idempotency obstruction -- the only additive idempotent (2a=a) is 0, so OR's universal a|a=a cannot map to +. The "everything-key incl. PQ" is provably EMPTY; the boundary IS the result.
