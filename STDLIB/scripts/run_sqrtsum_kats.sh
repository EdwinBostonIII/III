...
run 2182_march_v2       99 "$OUT/lattice_march.o" "$OUT/sqrt_sum_sign.o" "$OUT/kfield.o" "$LIB"  # RATIONAL-DIRECTION MARCH v2 (charter increment 4): ANY integer direction via the DHAT-refined-lattice isomorphism (every event time/position INTEGER) -- 12000 crossings d=(5,7) with the PARITY THEOREM certifying zero corners + exact Sturmian state (1050055,1470000); 12000 reversed crossings land BIT-EXACTLY on the first forward crossing; exact corner double-step d=(2,3) vs the x-first biased twin entering the sum-4 cell the ray never touches; general-d sphere: disc==0 CERTIFIED tangent / hit / miss on one geometry
