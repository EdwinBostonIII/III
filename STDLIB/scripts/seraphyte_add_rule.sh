#!/usr/bin/env bash
# seraphyte_add_rule.sh -- AUTONOMOUS addition of a strength-reduction rule through the FULL Seraphyte loop:
#   BASELINE(rule absent: x*V0 -> imul) -> PROVE (bv_ring identities, wrong decomposition refuted) ->
#   EMIT (seraphyte_emit_rule.sh, no human) -> REBUILD -> EMISSION CHECK (family factors reduce; sibling
#   rules UNCHANGED) -> FULLY-FUNCTIONAL (emitted x*v vs a memory-load IMUL reference over overflow edges) ->
#   FIXPOINT (iiis-2 == iiis-3) -> CERT (multi-engine cor_selftest) -> ACCEPT | ROLLBACK byte-exact.
#
# UNIFIED by the reunification (III-REUNIFICATION-PLAN W2.3): seraphyte_add_2shift.sh and seraphyte_add_2sub.sh
# were one spine instantiated twice; the rule DESCRIPTOR below is now the only difference.  "Fully functional"
# is the VALUE, not the mnemonics: the reference is x*MULS[i] (a MEMORY-LOAD multiply the compiler cannot
# strength-reduce -> a real imul) compared against the literal multiply (the emitted rule).
#
# Usage:  seraphyte_add_rule.sh <2sh|2shift|2ss|2sub>
set -uo pipefail
...
RULE="${1:?usage: seraphyte_add_rule.sh <2sh|2shift|2ss|2sub>}"
[ "$RULE" = "2shift" ] && RULE="2sh"
[ "$RULE" = "2sub" ]   && RULE="2ss"

case "$RULE" in
    2sh)
        TAG="add2sh";  GUARD="cgopt_mul_2sh_admit";  V0=10
        EMIT_ARG="2sh"
        CHECK_VS="10 6 12 20";  CHECK_OP="shl"
        ACCEPT_MSG="cg_r3 GAINED the 2-shift rule (x*10 -> two shifts), proven + fully-functional + fixpoint + cert green." ;;
    2ss)
        TAG="add2ss";  GUARD="cgopt_mul_2ss_admit";  V0=14
        EMIT_ARG="2ss"
        CHECK_VS="14 28 30 56 60";  CHECK_OP="sub"
        ACCEPT_MSG="cg_r3 GAINED the 2-shift-sub family (x*14 -> two shifts + sub); the emitter keeps producing." ;;
    *) echo "[add] unknown rule '$RULE'"; exit 2 ;;
esac

W="$ROOT/STDLIB/build/_$TAG"; mkdir -p "$W"
say(){ printf '[%s] %s\n' "$TAG" "$*"; }
hr(){  printf '%s\n' "------------------------------------------------------------"; }
...
fn chk(x: u64) -> u32 {
    if (x * 10u64) != ref(x, 0u64) { return 1u32 }
...
cd "$ROOT"
hr; say "STEP 0 -- BASELINE ($RULE-absent): x*$V0 should be 'imul' and the rule predicate must be absent"
[ "$(grep -c "$GUARD" COMPILER/BOOT/cg_opt_rules.iii)" = "0" ] || { say "FAIL: rule already present (not a clean add)"; exit 1; }
BASE0="$(emit_xv $V0)"
printf '%s' "$BASE0" | grep -qi imul || { say "FAIL: baseline x*$V0 is not imul ($BASE0)"; exit 1; }
say "  baseline: x*$V0 -> $BASE0 (imul, as expected)"

hr; say "STEP 1 -- PROVE the identity (bv_ring); a wrong decomposition must be refuted"
write_prove
...
[ "$PR" = "99" ] && say "  PROVEN (bv_ring): the $RULE identity holds for the family; wrong decomposition refuted." || { say "FAIL: proof rc=$PR"; exit 1; }

hr; say "STEP 2 -- EMIT: the unified patch-emitter writes the rule's source (no human)"
bash STDLIB/scripts/seraphyte_emit_rule.sh "$EMIT_ARG" 2>&1 | grep -E '^\[emit\]' | sed 's/^/  /'

hr; say "STEP 3 -- REBUILD iiis-2 with the emitted rule"
rebuild_iiis2
[ -x "$IIIS" ] || { say "FAIL: iiis-2 did not rebuild"; git checkout HEAD -- "${F3[@]}"; exit 2; }

hr; say "STEP 4 -- EMISSION CHECK: family factors reduce; sibling rules UNCHANGED (dispatch disjoint)"
ok=1
for v in $CHECK_VS; do a="$(emit_xv $v)"; printf '%s' "$a" | grep -qi imul && { say "  x*$v still imul ($a) -- FAIL"; ok=0; }
  printf '%s' "$a" | grep -qi "$CHECK_OP" || { say "  x*$v no $CHECK_OP ($a) -- FAIL"; ok=0; }; done
if [ "$RULE" = "2ss" ]; then
...
if [ "$ok" = "1" ]; then say "  x*$V0 -> $(emit_xv $V0); x*7 -> $R7 (subk); x*9 -> $R9 (shladd); x*8 -> $R8 (pow2) -- all correct"
...
[ "$FF" = "99" ] && say "  VALUE PROVEN: the family multiplies match the imul reference on all edges" \
...
if [ "$FIX" = "59 passed, 0 failed" ] && [ "$RC" = "99" ]; then
  hr; say "DECISION: ACCEPT -- $ACCEPT_MSG"
  hr; exit 0
