...
int main() {
    ...
    uint8_t out[32];
    cl_sha_init(&ctx);
    cl_sha_update(&ctx, "abc", 3);
    cl_sha_final(&ctx, out);
    /* SHA-256("abc") = ba7816bf 8f01cfea 414140de 5dae2223 b00361a3 96177a9c b410ff61 f20015ad */
    if (out[0]!=0xba) { return 1; }   if (out[1]!=0x78) { return 2; }   if (out[12]!=0x5d) { return 3; }
    if (out[3]!=0xbf) { return 4; }   if (out[8]!=0x41) { return 5; }  if (out[28]!=0xf2) { return 6; }
    if (out[31]!=0xad) { return 7; }  if (out[16]!=0xb0) { return 8; }
    return 99;
}
