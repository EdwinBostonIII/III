...
rcp=$(cfeat test_compound.c)
if [ "$rcp" = "ok" ]; then say "ccsv SEED-DDC #5 : compound assignment (local x op= e: += -= *= |= &= ^= <<= >>=) -- detected as <binop> followed by = (no new lexer tokens), lowered to GET;e;OP;SET with width-mask via emit_lset (uint32 wrap: 0xFFFFFFFF+=1 -> 0) -> all 99.  (field/array compound c->h[i]+=a is the next step for ceiling.c.)"; else say "FAIL seed-ddc compound: $rcp"; fail=1; fi
