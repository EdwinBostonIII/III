...
rp=$(cfeat test_ptr.c); rs=$(cfeat test_struct.c); rt=$(cfeat test_td.c); re=$(cfeat test_enum.c); ri=$(cfeat main_inc.c)
if [ "$rp" = "ok" ] && [ "$rs" = "ok" ] && [ "$rt" = "ok" ] && [ "$re" = "ok" ] && [ "$ri" = "ok" ]; then
  say "ccsv C TIERS : arrays+char+pointers + structs + typedef/union/p->f + enum/#define/typedef-ptr + MULTI-FILE #include (main_inc.c pulls struct/fn/#define from util.h) -> sovereign x86 + wasm + verifier + gcc, all 99."
else say "FAIL tiers: ptr=$rp struct=$rs td=$rt enum=$re include=$ri"; fail=1; fi
