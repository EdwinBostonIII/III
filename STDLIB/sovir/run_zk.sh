...
fail=0; say(){ echo "[zk] $*"; }
for m in svir_x86 svir_wasm iiisv zk_svir_exec zk_svir_add zk_svir_range; do "$IIIS" "$S/$m.iii" --compile-only --out "$W/$m.o" >/dev/null 2>&1 || { say "FAIL compile $m"; fail=1; }; done
...
gcc "$W/zk_svir_range.o" "$LIB" -lkernel32 -o "$W/zk_svir_range.exe" 2>/dev/null
timeout 30 "$W/zk_svir_range.exe" >/dev/null 2>&1; rrc=$?
if [ $rrc -eq 99 ]; then say "zkVM-RANGE : 14-bit limb range-check (bit-decomposition AIR, NO lookup) ; valid limbs {0,10995,16383} pass + out-of-range {16384,20000} rejected -> 99"
