...
say "CONCRETE-SOUNDNESS [applies to EVERY 'SOUND' above]: all 'SOUND' = STRUCTURAL soundness (no prover can bypass the FS binding -- proven by the malicious-prover oracles). It is NOT yet PRODUCTION security. Field p~2^30, blowup 4, 16 FRI queries => concrete error ~2^-11 (FRI-query-limited), every algebraic+folding challenge CAPPED at ~2^-27 by the 30-bit field, Fiat-Shamir GRINDABLE at ~2^27. Production targets 80-128 bits; we deliver ~11-27. THE BOTTLENECK = the base field. The ZK-EXT2-*/ZK-EXT4 gadgets BUILD+VERIFY the extension-field TOOLS (GF(p^2)/GF(p^4) arithmetic, the FRI fold, the CP construction) but they are NOT YET LOAD-BEARING: no real gadget uses a query-based extension-field verifier, so EVERY real gadget's concrete soundness is STILL ~27-bit. The soundness-bearing work = lift air_stark_verify's QUERY layer (commit->challenge->open->verify-fold) to GF(p^2), proven by a forged-CP malicious arm; ALSO N=8/D=32 is too small for many independent queries (its own bottleneck). See DOCS/III-ZK-CONCRETE-SOUNDNESS.md. (advisor-surfaced; the asterisk is the honest word)"
if [ $fail -eq 0 ]; then
