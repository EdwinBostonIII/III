...
if [ "$elivrc" = "99" ]; then say "ZK-EXT2-LIVE : the EXTENSION-FIELD STARK on a REAL constraint -- a trace with the nonlinear transition next=cur^2 ([2,4,16,256]) run through the LIVE air (air_reset/set_trace/add_term/build_lde) into a GF(p^2) composition polynomial (air_build_cp_ext2: the combination uses a GF(p^2) challenge, the Z_T division is per-limb NTT), then VERIFIED by the GF(p^2) consistency check CP[j]*Z_T(omega^j)==combine(omega^j) at every LDE point + the GF(p^2) FRI fold -> the satisfying trace ACCEPTS, a violating trace ([2,4,17,256], row 1 broken) is REJECTED (combine!=0 at the tampered row) -> 99. The verified GF(p^2) core is now wired to the REAL prover; the base-field air path is untouched (additive). NEXT: GF(p^2) Merkle+queries (succinctness) + tower to GF(p^4) ~2^120 + migrate the live gadgets."
...
if [ "$ext4rc" = "99" ] && [ "$nrrc" = "2" ]; then say "ZK-EXT4 : GF(p^4) = GF(p^2)[v]/(v^2-(2+u)) ~2^120 arithmetic VERIFIED (g=2+u is a genuine GF(p^2) NON-RESIDUE: the Legendre probe found g^((p^2-1)/2)==-1 at c=2) -- v^2==g, a*a^-1==1 (would fail on a zero divisor), distributivity, associativity all hold -> 99. The PRODUCTION-bit-count FS field: the combination + FRI-fold challenges over GF(p^4) give Schwartz-Zippel error ~deg/p^4 ~2^-114, PAST the 80-128 bit production threshold (GF(p^2)'s ~2^-57 is below). NEXT: route the live air_build_cp_ext path's challenges through GF(p^4) (4 limbs) + the succinct FRI."
