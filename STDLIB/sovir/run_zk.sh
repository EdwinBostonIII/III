...
# (12) ZK-PERM-K3PROD: THE PERM-BIT LIFT -- the memory permutation made PRODUCTION-sound (~2^-81) via k=3 repeated challenges.
k3rc=$(runzk zk_perm_k3prod)
if [ "$k3rc" = "99" ]; then say "ZK-PERM-K3PROD : *** THE PERM-BIT LIFT *** -- the memory zkVM's permutation made PRODUCTION-sound. A grand-product permutation (program order == sorted order iff equal products at a random challenge) run under k=3 INDEPENDENT FS-derived challenges (AIR_PERM_A, R2, R3 = distinct keccak-digest slices of the committed v columns): a non-permutation must collide at ALL THREE to pass -> ~(n/p)^3 ~2^-81 = PRODUCTION (one base-field alpha was only ~2^-27); FS-bound (challenges derived AFTER the prover commits v) so grinding is ~2^81. AIR W=8 (vp,vs + 6 accumulators), N=64: 6 grand-product recurrences VERIFIED over the GF(p^4) production CP (line-755 + D=256 GF(p^4) FRI at 128 queries, ~2^-86) + 3 boundaries acp_k[last]==acs_k[last]. HONEST perm (vs=reversed vp) ACCEPTS; FORGED non-perm (one element tampered) -- recurrences still verify but the k=3 BOUNDARIES FAIL -> REJECTED -> 99. So the memory permutation is PRODUCTION-sound: overall ~min(2^-86 query, 2^-114 challenge, 2^-81 perm) = ~2^-81. Regression green (additive R2/R3 + perm kinds 4/5). NEXT: assemble the full FUSED (compute+memory+control + read-consistency + sortedness + this k=3 perm) in one N=64 GF(p^4) gadget."
else say "FAIL ZK-PERM-K3PROD: zk_perm_k3prod=$k3rc (1=honest-boundary-fail 2=honest-verify-fail 4=forged-verify-fail 3=forged-boundary-ACCEPTED)"; fail=1; fi
